...
export default function PendingTX({walletAddress,allPendingTX,modalOpen,setModal}) {
    ...
    const [transactionErrorMessage,settransactionErrorMessage] = useState("");
     ...
     const handleApproveTX = async (_index) => {
        ...
        let requestResult = await wallet.transactionApproval(_index);
     ...
     const getNumberFromWei = (wNum) => {
          return Web3.utils.fromWei(wNum.toString(), 'ether');
     }
                    ...
                    <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                      <div className="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
                        <h5 className="text-xl font-medium leading-normal text-gray-800" id="exampleModalScrollableLabel">
                          Confirm Pending Transaction Request
                        </h5>
                        <button type="button"
                            ...
                            <div className="flex flex-col">
                                <div className="flex flex-col">
                                    <table>
                                        ...
                                        <tbody>
                                            {allPendingTX.map((pt,idx)=>{
                                                return(
                                                    <tr className="m-2" key={idx}>
                                                        <td className="border text-xs">{getNumberFromWei(pt.value)}</td>
                                                        <td className="border text-xs">{pt[0]}</td>
                                                        <td className="border text-xs">{pt?.approved?"Approved":"Pending"}</td>
                                                        <td className="border text-xs">
                                                            <button className="mt-1 text-green-500 px-2 py-1 border border-gray-400 rounded text-xs" 
                                                                onClick={handleApproveTX.bind(this,idx)}
                                                            >
                                                                Approve 
                                                            </button>
                                                        </td>
