...
export default function AddOwnerApproval({walletAddress,modalOpen,setModal}) {
    ...
    const [transactionErrorMessage,settransactionErrorMessage] = useState("");
     ...
     const handleApproveAddOwnerRequest = async () => {
        settransactionUnderProcess(true);
            ...
            try {
                 let requestResult =  await wallet.addOwnerApproval(_index);
                 if(requestResult){
                    closeModal();
                    settransactionStatus("Approval for add Owner Request Was Sucessful");
                 }else{
                    settransactionStatus("Approval to add Owner has been Failed");
                 }
                    ...
                    <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                      <div className="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
                        <h5 className="text-xl font-medium leading-normal text-gray-800" id="exampleModalScrollableLabel">
                          Confirm Pending Add Owner Request
                        </h5>
                        <button type="button"
